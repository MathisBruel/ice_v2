...
void SeamCarvingThread::run()
{
    try {
        ...
        Poco::Logger::get("SeamCarvingThread").debug("Exception in seamCarvingThread !", __FILE__, __LINE__);
        Poco::Logger::get("SeamCarvingThread").debug(e.what(), __FILE__, __LINE__);
    }
}