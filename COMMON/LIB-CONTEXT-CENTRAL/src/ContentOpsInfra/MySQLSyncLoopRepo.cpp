#include "ContentOpsInfra/MySQLSyncLoopRepo.h"
#include "ResultQuery.h"
#include "ContentOpsInfra/MySQLDBConnection.h"

std::string MySQLSyncLoopRepo::_database = "ice";
std::string MySQLSyncLoopRepo::_table = "loopSync";

std::unique_ptr<Query> MySQLSyncLoopRepo::MySQLcreate(COD_SyncLoop* syncloop){
    _syncloopIds = syncloop->GetSyncLoopId();
    if (_syncloopIds[0] == -1 || _syncloopIds[1] == -1 || _syncloopIds[2] == -1 || _syncloopIds[3] == -1) { return nullptr; }

    _syncLoopPath = syncloop->GetSyncLoopPath();

    auto createQuery = std::make_unique<Query>(Query::INSERT, _database, _table);
    createQuery->addParameter("id_serv_pair_config", (void*)&_syncloopIds[0], "int");
    createQuery->addParameter("id_content", (void*)&_syncloopIds[1], "int");
    createQuery->addParameter("id_type", (void*)&_syncloopIds[2], "int");
    createQuery->addParameter("id_localisation", (void*)&_syncloopIds[3], "int");
    createQuery->addParameter("path_sync_loop", &_syncLoopPath, "string");
    return createQuery;
}
std::unique_ptr<Query> MySQLSyncLoopRepo::MySQLread(COD_SyncLoop* syncloop){
    _syncloopIds = syncloop->GetSyncLoopId();
    auto readQuery = std::make_unique<Query>(Query::SELECT, _database, _table);
    readQuery->addParameter("id_serv_pair_config", nullptr, "int");
    readQuery->addParameter("id_content", nullptr, "int");
    readQuery->addParameter("id_type",nullptr, "int");
    readQuery->addParameter("id_localisation", nullptr, "int");
    readQuery->addParameter("path_sync_loop", nullptr, "string");
    if (*&_syncloopIds[0] != -1 ) { readQuery->addWhereParameter("id_serv_pair_config", (void*)&_syncloopIds[0], "int"); };
    if (*&_syncloopIds[1] != -1 ) { readQuery->addWhereParameter("id_content", (void*)&_syncloopIds[1], "int"); };
    if (*&_syncloopIds[2] != -1 ) { readQuery->addWhereParameter("id_type", (void*)&_syncloopIds[2], "int"); };
    if (*&_syncloopIds[3] != -1 ) { readQuery->addWhereParameter("id_localisation", (void*)&_syncloopIds[3], "int"); };
    return readQuery;
}
std::unique_ptr<Query> MySQLSyncLoopRepo::MySQLupdate(COD_SyncLoop* syncloop){
    _syncloopIds = syncloop->GetSyncLoopId();
    if (_syncloopIds[0] == -1 || _syncloopIds[1] == -1 || _syncloopIds[2] == -1 || _syncloopIds[3] == -1) { return nullptr; }

    _syncLoopPath = syncloop->GetSyncLoopPath();

    auto updateQuery = std::make_unique<Query>(Query::UPDATE, _database, _table);
    updateQuery->addParameter("path_sync_loop", &_syncLoopPath, "string");
    updateQuery->addWhereParameter("id_serv_pair_config", (void*)&_syncloopIds[0], "int");
    updateQuery->addWhereParameter("id_content", (void*)&_syncloopIds[1], "int");
    updateQuery->addWhereParameter("id_type", (void*)&_syncloopIds[2], "int");
    updateQuery->addWhereParameter("id_localisation", (void*)&_syncloopIds[3], "int");
    return updateQuery;
}
std::unique_ptr<Query> MySQLSyncLoopRepo::MySQLremove(COD_SyncLoop* syncloop){
    _syncloopIds = syncloop->GetSyncLoopId();
    if (_syncloopIds[0] == -1 || _syncloopIds[1] == -1 || _syncloopIds[2] == -1 || _syncloopIds[3] == -1) { return nullptr; }

    auto removeQuery = std::make_unique<Query>(Query::REMOVE, _database, _table);
    removeQuery->addWhereParameter("id_serv_pair_config", (void*)&_syncloopIds[0], "int");
    removeQuery->addWhereParameter("id_content", (void*)&_syncloopIds[1], "int");
    removeQuery->addWhereParameter("id_type", (void*)&_syncloopIds[2], "int");
    removeQuery->addWhereParameter("id_localisation", (void*)&_syncloopIds[3], "int");
    return removeQuery;
}

std::unique_ptr<ResultQuery> MySQLSyncLoopRepo::getSyncLoops() {
    std::unique_ptr<MySQLDBConnection> dbConn = std::make_unique<MySQLDBConnection>();
    dbConn->InitConnection();
    std::unique_ptr<Query> query = std::make_unique<Query>(Query::SELECT, _database, _table);
    query->addParameter("id_serv_pair_config", nullptr, "int");
    query->addParameter("id_content", nullptr, "int");
    query->addParameter("id_type", nullptr, "int");
    query->addParameter("id_localisation", nullptr, "int");
    query->addParameter("path_sync_loop", nullptr, "string");
    std::unique_ptr<ResultQuery> result(dbConn->ExecuteQuery(query.get()));
    return result;
}