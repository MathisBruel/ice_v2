...
COB_Syncs COB_SyncRepo::GetSyncsByRelease(int contentId, int typeId, int localisationId)
{
    COB_Syncs syncs;
    std::unique_ptr<ResultQuery> result = _syncRepo->getSyncsByRelease(contentId, typeId, localisationId);
    if (!result || !result->isValid()) {
        throw std::runtime_error("Failed to get syncs by release: " + (result ? result->getErrorMessage() : "null result"));
    }
    int nbRows = result->getNbRows();
    syncs.reserve(nbRows);
    for (int i = 0; i < nbRows; ++i) {
