...
MovingHead::MovingHead(std::string name, std::string id, uint32_t serial, uint16_t dmxOffset, std::string formatBuffer, 
    int panFullAngle, int tiltFullAngle, double biasPan, double biasTilt, uint8_t defaultZoom, int lumens)
    : DmxDevice(name, id, serial, dmxOffset, formatBuffer)
{
    type = MOVING_HEAD;
    this->panFullAngle = panFullAngle;
    this->tiltFullAngle = tiltFullAngle;
    this->biasPan = biasPan;
    this->biasTilt = biasTilt;
    this->defaultZoom = defaultZoom;
    this->lumens = lumens;
}
    
void MovingHead::createPayload(uint16_t red, uint16_t green, uint16_t blue, uint16_t white, uint8_t intensity, uint8_t zoom, double pan, double tilt, bool lutApplied)
{
    if (lutApplied) {
    ...
    // -- transform to device
    double paddingPan = (double)(0xffff) / (double)(panFullAngle);
    double paddingTilt = (double)(0xffff) / (double)(tiltFullAngle);
    uint16_t panF = (uint16_t)((pan + ((double)(panFullAngle) / 2.0)) * paddingPan + 0.5);
    uint16_t tiltF = (uint16_t)((tilt + ((double)(tiltFullAngle) / 2.0)) * paddingTilt + 0.5);
    ...
    for (int i = 0; i < bufferTemplate.length(); i++) {
        ...
        else if (bufferTemplate.substr(i, 1) == "P") {
            payload[i] = (uint8_t)(panF >> 8);
        }
        else if (bufferTemplate.substr(i, 1) == "p") {
            payload[i] = (uint8_t)(panF & 0xff);
        }
        else if (bufferTemplate.substr(i, 1) == "T") {
            payload[i] = (uint8_t)(tiltF >> 8);
        }
        else if (bufferTemplate.substr(i, 1) == "t") {
            payload[i] = (uint8_t)(tiltF & 0xff);
        }
    }
}

void MovingHead::createPayload(uint16_t red, uint16_t green, uint16_t blue, uint16_t white, uint8_t intensity, double pan, double tilt, bool lutApplied)
{
    if (lutApplied) {
        red = std::min((int)((double)(red * 0.72) + 0.5), 255);
        blue = std::min((int)((double)(blue * 0.5) + 0.5), 255);
